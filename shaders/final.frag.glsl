...
//   out0.rgb = pow(out0.rgb, vec3(u_gamma * 0.7));
    ...
    if (true) {
        ...
        float dots = exp(-0.9*length(gl_FragCoord.xy - physarum.xy));
        vec3 creature_color = hsl2rgb(mix(u_creatures_hsl, u_creatures_hsl_inside, pow(clamp(physarum.w*0.5, 0., 1.), 2.)));
       out0.rgb += vec3(dots * 4.) * creature_color * u_final_creatures; //creature_color*dots*u_final_creatures * 5.;//(0.7, 1, 0.9, 0,) (0.3, 0.7, 0.5, 0), alien shrimp (0.8, 0.01, 0.01, 0)
